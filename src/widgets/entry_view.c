...
entry_view_class_init (EntryViewClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, EntryView, entry_render);
...
rename_finished (GtkDialog *dialog, int response_id, gpointer user_data)
{
  ...
  GtkEntryBuffer *name_buffer = gtk_text_get_buffer (GTK_TEXT (name_input));
...
rename_button_clicked (GtkButton *button, gpointer user_data)
{
  ...
  text_input = gtk_text_new_with_buffer (entry_buffer);
...
entry_view_load_html (EntryView *self)
{
  ...
  gtk_text_buffer_set_text (buffer, text_md, -1);
  gtk_text_view_set_buffer (GTK_TEXT_VIEW (self->entry_render), buffer);
  g_object_set (self->entry_render, "valign", GTK_ALIGN_FILL, NULL);
